...
def generate_fee_visualizations(
    ...
    """
    Построение трёх интерактивных графиков по комиссиям:
      1. Daily Fees (линия)
      2. Cumulative Fees (кумулятивная линия)
      3. Calendar Heatmap (тепловая карта по дням недели и год-неде́лям)
    ...
    """
    # 0) Копируем и приводим дату к datetime
    df = vol_df.copy()
    ...
    # 3) Calendar Heatmap
    #   — приводим к равномерному daily frequency, заполняем нулями
    cal_df = (
        df
        .set_index(date_col)
    ...
    # Получаем ISO-годы и ISO-недели
    iso = cal_df.index.to_series().dt.isocalendar()
    # Склеиваем «год-неделя» в строку для уникальных колонок
    cal_df['year_week'] = iso['year'].astype(str) + "-W" + iso['week'].apply(lambda w: f"{w:02d}")
    cal_df['weekday']   = cal_df.index.weekday  # Mon=0 … Sun=6

    # Пивотим: строки — weekday, колонки — year_week
    ...
    fig_heatmap = px.imshow(
        heatmap_df,
        labels=dict(x='Year-Week', y='Weekday', color='Fees (USD)'),
